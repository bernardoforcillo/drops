...
func TestQueryTagsReachTheDriver(t *testing.T) {
	...
	if _, err := db.Insert(tbl).Row(name.Val("Alice")).Exec(ctx); err != nil {
		t.Fatal(err)
	}
...
func TestUntaggedContextLeavesTheStatementAlone(t *testing.T) {
	...
	if _, err := pg.New(fd).Query(context.Background(), sql, 1); err != nil {
		t.Fatal(err)
	}
	if fd.queries[0] != sql {
...
// grepping the log for a statement seen in pg_stat_statements fails.
func TestHookSeesTheStatementTheServerSaw(t *testing.T) {
	fd := &fakeDriver{}
	var logged []string
	db := pg.New(fd).WithHook(func(_ context.Context, e drops.QueryEvent) {
		if e.SQL != "" {
