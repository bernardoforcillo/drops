...
// no tenant at all and without refusing, DELETE FROM "users" — no
// predicate, every tenant's rows, and nothing walks a DELETE back.
// The same snapshot dropped a soft-delete guard and an OnInsert hook
// registered after the alias was taken, so the alias read rows the
// application had deleted and wrote rows the hooks never touched.
//
...
// drops with sqlite.FilterTenant while leaving every other guard standing.
...
//
// A SQLite table carries no schema qualifier, so the name is the whole
// of it — but the question is asked through this function anyway, and
// not by reading t.name at the call sites, because the two other
// dialects answer it differently and a reader comparing them should
// find the same function in each.
