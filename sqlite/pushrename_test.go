...
// that it is the only thing standing between a rename and a rebuild
// that silently leaves the column behind — is a server behaviour and
// lives in the integration suite.

func TestDeclaredRenamesReadsColumnAndTable(t *testing.T) {
	people := sqlite.NewTable("people").RenamedFrom("users")
	sqlite.Add(people, sqlite.BigInt("id").PrimaryKey())
	sqlite.Add(people, sqlite.Text("emailAddress").NotNull().RenamedFrom("email"))
	sqlite.Add(people, sqlite.Text("nickname"))

	got := sqlite.DeclaredRenames(sqlite.NewSchema(people))
	want := []sqlite.RenameDecision{
		{Rename: sqlite.Rename{Kind: sqlite.RenameColumn, Table: "people", From: "email", To: "emailAddress"}, IsRename: true},
		{Rename: sqlite.Rename{Kind: sqlite.RenameTable, From: "users", To: "people"}, IsRename: true},
	}
	if len(got) != len(want) {
	...
	if got := sqlite.DeclaredRenames(renameUsersBefore()); len(got) != 0 {
...
func TestGenerateTakesTheRenameTheSchemaDeclares(t *testing.T) {
	...
	if body, ok := files[sqlite.RenameLogFile]; ok {
...
func TestGenerateRecordsAnAnswerThatProducedNoStatements(t *testing.T) {
	...
	fsys["drizzle/meta/_renames.json"] = &fstest.MapFile{Data: body}
	if _, err := sqlite.GenerateMigration(sqlite.GenerateOptions{
...
func TestPushRenameAdviceReplacesTheGeneratorsWording(t *testing.T) {
	candidates := []sqlite.RenameCandidate{{
		Rename:   sqlite.Rename{Kind: sqlite.RenameColumn, Table: "users", From: "email", To: "emailAddress"},
		FromType: "TEXT", ToType: "TEXT",
	}}
	generated := (&sqlite.RenameAmbiguityError{Candidates: candidates}).Error()
	...
	pushed := (&sqlite.RenameAmbiguityError{Candidates: candidates, Advice: "answer it in the schema"}).Error()
