...
func TestUntaggedContextLeavesTheStatementAlone(t *testing.T) {
	...
	if _, err := mysql.New(fd).Query(context.Background(), sql, 1); err != nil {
		t.Fatal(err)
	}
	if fd.queries[0] != sql {
...
func TestHookSeesTheStatementTheServerSaw(t *testing.T) {
	fd := &fakeDriver{}
	var logged []string
	db := mysql.New(fd).WithHook(func(_ context.Context, e drops.QueryEvent) {
		if e.SQL != "" {
