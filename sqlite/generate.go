...
// that case. The one thing a no-op run still writes is
...
func GenerateMigration(opts GenerateOptions) (*GenerateResult, error) {
	...
	if opts.Dir == "" {
		return nil, errors.New("drops/sqlite: Dir is required")
	}
	if opts.Now == nil {
	...
	if err := opts.Write(tag+".sql", []byte(sql)); err != nil {
		return nil, fmt.Errorf("drops/sqlite: write migration SQL: %w", err)
	}
	if downSQL != "" {
		if err := opts.Write(tag+".down.sql", []byte(downSQL)); err != nil {
			return nil, fmt.Errorf("drops/sqlite: write down SQL: %w", err)
		}
	}
	if err := opts.Write(fmt.Sprintf("meta/%04d_snapshot.json", idx), snapshotBytes); err != nil {
		return nil, fmt.Errorf("drops/sqlite: write snapshot: %w", err)
	}
...
func loadPrevSnapshot(fsys fs.FS, dir string) (*Snapshot, int, error) {
	...
	if err != nil {
		...
		return nil, 0, fmt.Errorf("drops/sqlite: read meta dir: %w", err)
	}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("drops/sqlite: read snapshot %s: %w", latest.name, err)
	}
...
func loadJournalForWrite(fsys fs.FS, dir string) (*drizzleJournal, error) {
	...
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return &drizzleJournal{Version: "7", Dialect: "sqlite"}, nil
		}
		return nil, fmt.Errorf("drops/sqlite: read journal: %w", err)
	}
	var j drizzleJournal
	if err := json.Unmarshal(body, &j); err != nil {
		return nil, fmt.Errorf("drops/sqlite: parse journal: %w", err)
	}
	return &j, nil
}
