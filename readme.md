...
service ends up writing by hand:

- **Change data capture from the WAL** — logical replication slots
  ...
  to an outbox too.
- **Precise query-cache invalidation** — a query declares the topics
  ...
  TTL.
- **Plan hints and plan assertions** — `pg_hint_plan` directives
...
- **Selectivity estimates and bounded-memory scans** — a Count-Min
