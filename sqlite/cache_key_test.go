...
// apart from another's. The tenant predicate is AND-ed into the
// statement by the entity, so two tenants asking the same question send
// byte-identical SQL and differ in one bound argument — the argument
// this key has to preserve. These are the properties that make it fit
// to hold that boundary; each of them was false while the key was
// sha256(sql + "%v" per arg) truncated to sixteen hex digits.
func TestQueryKeyIsBuiltForIsolation(t *testing.T) {
	const table = "projects"
	sql := `SELECT * FROM "projects" WHERE ("projects"."tenantId" = ?)`
	seven := int64(7)
	otherSeven := int64(7)
	...
	// Two statements of the same length must not be able to trade text
	// for arguments either: the SQL is length-prefixed like the values.
	if got, want := queryKey(table, "SELECT 1", []any{"x"}) == queryKey(table, "SELECT 1x", nil), false; got != want {
		t.Errorf("sql and a trailing argument produce one key: got = %v, want %v", got, want)
	}

	// The digest is not truncated. At 64 bits a collision is expected
	// around 2^32 entries, a population a busy cache reaches, and a
	// collision here answers one tenant's query with another tenant's
	// rows.
...
// a NULL parameter is ordinary in a WHERE clause and must not panic the
// key builder that every cached read goes through.
func TestQueryKeyHandlesNilArguments(t *testing.T) {
	var nilPtr *int64
	sql := `SELECT * FROM "projects" WHERE ("projects"."name" = ?)`
	keys := map[string]string{
		"untyped nil": queryKey("projects", sql, []any{nil}),
		"nil pointer": queryKey("projects", sql, []any{nilPtr}),
		"zero value":  queryKey("projects", sql, []any{int64(0)}),
	}
