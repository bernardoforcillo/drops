...
// out quoted — see drops.Builder.WriteIdent — and ClickHouse compares
// an identifier byte for byte whether it is quoted or not, so
// `tenantId` and `TenantId` are two columns: a handle spelled the
// second way names a column the table does not have, the server
// answers UNKNOWN_IDENTIFIER, and folding here would instead refuse a
// schema that legitimately declares both.
//
