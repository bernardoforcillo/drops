...
// moneyTypeName / moneyPkgPath identify mysql.Money so AutoTable maps
...
//
// MySQL has no serial family — autoIncrement simply keeps INTEGER; pair
...
func populateTable[T any](tbl *Table) {
	...
	if rt.Kind() != reflect.Struct {
		panic(fmt.Sprintf("drops/mysql: AutoTable requires T to be a struct, got %s", rt.Kind()))
	}
...
func walkAutoFields(rt reflect.Type, fn func(reflect.StructField, autoOpts)) {
	for i := 0; i < rt.NumField(); i++ {
		...
		if err != nil {
			panic(fmt.Sprintf("drops/mysql: AutoTable %s.%s: %v", rt.Name(), f.Name, err))
		}
		fn(f, opts)
	}
}
...
func autoColumnType(structName, fieldName string, ft reflect.Type) ColumnType {
	...
	panic(fmt.Sprintf("drops/mysql: AutoTable %s.%s: unsupported field type %s — declare the column by hand",
