...
// Relations declaration layer, mirroring drops/pg and drops/sqlite. A
// Relation names an association between two tables so Find().With(name)
// can eager-load it with a single batched query per edge (implemented
// in find.go).
//
// A Table.Relation / Table.Rel pair and a relations map lived in
// table.go once, against a *Relation type no file in the package ever
// defined: it had never compiled, nothing could populate the map, and
// there was no loader for an edge to feed. This is that declaration
// layer written for real, with find.go as the loader it feeds.
