...
//
//	seeder := mysql.NewSeeder(db)
//	mysql.SeedAdd(seeder, UserEntity, alice, bob)
//	mysql.SeedAdd(seeder, PostEntity, post1, post2)
	...
	run := func(db *DB) error {
		for i, op := range s.ops {
			if err := op.run(db, ctx); err != nil {
				return fmt.Errorf("drops/mysql: Seeder op #%d: %w", i, err)
			}
		}
		return nil
	}
