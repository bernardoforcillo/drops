...
func ccTable(name string) (*mysql.Table, *mysql.Entity[ccRow]) {
	t := mysql.NewTable(name)
	mysql.Add(t, mysql.BigInt("id").PrimaryKey())
	tenant := mysql.Add(t, mysql.BigInt("tenantId").NotNull())
	mysql.Add(t, mysql.Text("name"))
	t.ContextFilter(mysql.TenantFilter(tenant))
	return t, mysql.NewEntity[ccRow](t, mysql.AllowNullableColumns("name"))
}
...
func TestTheQueryCacheKeyCarriesTheResolvedTenant(t *testing.T) {
	...
	theirs, err := ent.Query(db).All(mysql.WithTenant(context.Background(), int64(5)))
	if err != nil {
		t.Fatalf("All: %v", err)
	}
	if len(drv.SQL()) != 2 {
...
func TestTheQueryCacheRefusesWithoutATenant(t *testing.T) {
	...
	if _, err := ent.Query(db).All(context.Background()); !errors.Is(err, mysql.ErrTenantMissing) {
