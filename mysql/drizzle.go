...
// DrizzleTable is the migration history table. MySQL has no schemas, so
...
func (d *DrizzleMigrator) loadJournal() (*drizzleJournal, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("drops/mysql: read drizzle journal %q: %w", p, err)
	}
	var j drizzleJournal
	if err := json.Unmarshal(body, &j); err != nil {
		return nil, fmt.Errorf("drops/mysql: parse drizzle journal: %w", err)
	}
	if j.Dialect != "" && j.Dialect != "sqlite" {
		return nil, fmt.Errorf("drops/mysql: drizzle journal dialect is %q; only sqlite is supported", j.Dialect)
	}
	return &j, nil
}
...
func (d *DrizzleMigrator) LoadEntries() ([]DrizzleEntry, error) {
	...
	for _, e := range j.Entries {
		...
		if err != nil {
			return nil, fmt.Errorf("drops/mysql: read migration %s.sql: %w", e.Tag, err)
		}
...
// ensureSchema creates the migration history table (MySQL has no
...
func (d *DrizzleMigrator) Up(ctx context.Context) error {
	...
	for _, e := range entries {
		...
		if err := d.applyOne(ctx, e); err != nil {
			return fmt.Errorf("drops/mysql: applying drizzle migration %s: %w", e.Tag, err)
		}
	}
	return nil
}
