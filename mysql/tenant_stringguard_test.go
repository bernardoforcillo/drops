...
func textAxisTable(name string) (*mysql.Table, *mysql.Col[string], *mysql.Col[string]) {
	t := mysql.NewTable(name)
	tenant := mysql.Add(t, mysql.Text("tenantId").NotNull())
	title := mysql.Add(t, mysql.Text("title").NotNull())
	t.ContextFilter(mysql.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
...
// losing nothing.
func blobAxisTable(name string) (*mysql.Table, *mysql.Col[[]byte], *mysql.Col[string]) {
	t := mysql.NewTable(name)
	tenant := mysql.Add(t, mysql.Blob("tenantId").NotNull())
	title := mysql.Add(t, mysql.Text("title").NotNull())
	t.ContextFilter(mysql.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
	return t, tenant, title
}
...
func TestANamedStringTypeIsTheSameTenant(t *testing.T) {
	...
	st := drv.Last()
	wantText(t, st.SQL, "INSERT INTO `guard_named_rows` (`title`, `tenantId`) VALUES (?, ?)")
	wantArgs(t, st.Args, "x", "acme")
}
