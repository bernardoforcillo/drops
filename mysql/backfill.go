...
//
//	bf := mysql.NewBackfill(db, "userRegionFill").
//	    ChunkSize(10_000).
...
//	    }).
//	    Process(func(ctx context.Context, tx *mysql.DB, ids []int64) error {
...
// table are stored as Unix seconds (INTEGER) to sidestep MySQL's
...
func (b *Backfill) Run(ctx context.Context) error {
	if b.fetch == nil {
		return errors.New("drops/mysql: Backfill.Fetch not set")
	}
	if b.process == nil {
		return errors.New("drops/mysql: Backfill.Process not set")
	}
