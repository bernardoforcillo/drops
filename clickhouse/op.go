...
// WHERE clause, a row policy AND-ed with both. A closure here holds
// every one of them where resolveExpr cannot walk to it, so a subquery
// in any of them renders unscoped.
...
// rather than the interface so a caller that has to finish it off can,
