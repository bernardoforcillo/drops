...
func (s *Saga) Step(name string, forward, compensate SagaStepFn) *Saga {
	if forward == nil {
		panic("drops/mysql: Saga.Step requires a non-nil forward function")
	}
...
func (e *SagaError) Error() string {
	msg := fmt.Sprintf("drops/mysql: saga %q step %d (%s) failed: %v",
...
// reverse order.
//
// A step's transaction is a real one for DATA. It is not one for DDL:
// MySQL commits the open transaction when it meets CREATE, ALTER, DROP
// or TRUNCATE, so a step that changes the schema and then fails has
// already committed the change, and its compensation is the only thing
// that will undo it. Keep DDL out of a saga step, or write the
// compensation knowing it has to do the whole job. The same rule, and
// the same reason, as the note at the top of migrate.go.
