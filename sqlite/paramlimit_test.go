...
// One statement may carry at most 32766 bound parameters. It is a limit a bulk write
...
func plEntity(t *testing.T) (*sqlite.Entity[plRow], *dropstest.Driver, *sqlite.DB) {
	t.Helper()
	tbl := sqlite.NewTable("pl_rows")
	sqlite.Add(tbl, sqlite.BigSerial("id").PrimaryKey())
	for _, n := range []string{"a", "b", "c", "d", "e", "f", "g"} {
		sqlite.Add(tbl, sqlite.Text(n).NotNull())
	}
	drv := dropstest.New()
	return sqlite.NewEntity[plRow](tbl), drv, sqlite.New(drv)
}
...
func TestABatchOverTheParameterLimitIsRefusedBeforeItIsSent(t *testing.T) {
	...
	// so 4682 rows is 32774 parameters, over.
	rows := make([]plRow, 4682)
	for i := range rows {
		rows[i] = plRow{A: "a", B: "b", C: "c", D: "d", E: "e", F: "f", G: "g"}
	}

	_, err := ent.CreateMany(db, context.Background(), rows)
	if !errors.Is(err, sqlite.ErrTooManyParameters) {
	...
	for _, want := range []string{"32774", "32766", "batches"} {
...
func TestABatchAtTheParameterLimitIsSent(t *testing.T) {
	...
	// 4680 rows x 7 = 32760 parameters.
	rows := make([]plRow, 4680)
	for i := range rows {
		rows[i] = plRow{A: "a", B: "b", C: "c", D: "d", E: "e", F: "f", G: "g"}
	}
	...
	if n := len(drv.Statements()[0].Args); n != 32760 {
...
func TestAnOversizedInListIsRefusedToo(t *testing.T) {
	tbl := sqlite.NewTable("pl_reads")
	id := sqlite.Add(tbl, sqlite.BigInt("id").NotNull())
	drv := dropstest.New()
	db := sqlite.New(drv)

	ids := make([]int64, 37766)
	for i := range ids {
		ids[i] = int64(i)
	}
	var out []struct{}
	err := db.Select(id).From(tbl).Where(sqlite.In(id, ids)).All(context.Background(), &out)
	if !errors.Is(err, sqlite.ErrTooManyParameters) {
