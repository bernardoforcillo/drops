...
func typeCtx(v any) context.Context {
	return mysql.WithTenant(context.Background(), v)
}
...
func strAxisEntity() *mysql.Entity[strAxisRow] {
	tbl := mysql.NewTable("str_rows")
	mysql.Add(tbl, mysql.BigInt("id").PrimaryKey())
	tenant := mysql.Add(tbl, mysql.Text("tenantId").NotNull())
	mysql.Add(tbl, mysql.Text("title").NotNull())
	return mysql.NewEntity[strAxisRow](tbl).ScopeByTenant(tenant)
}
...
func numAxisEntity() *mysql.Entity[numAxisRow] {
	tbl := mysql.NewTable("num_rows")
	mysql.Add(tbl, mysql.BigInt("id").PrimaryKey())
	tenant := mysql.Add(tbl, mysql.BigInt("tenantId").NotNull())
	mysql.Add(tbl, mysql.Text("title").NotNull())
	return mysql.NewEntity[numAxisRow](tbl).ScopeByTenant(tenant)
}
...
// from both sides.
func wideAxisTable(name string) (tbl *mysql.Table, tenant *mysql.Col[int64], title *mysql.Col[string]) {
...
func narrowAxisTable(name string) (tbl *mysql.Table, tenant *mysql.Col[int32], title *mysql.Col[string]) {
	t := mysql.NewTable(name)
	mysql.Add(t, mysql.BigInt("id").PrimaryKey())
	tenantCol := mysql.Add(t, mysql.Integer("tenantId").NotNull())
	titleCol := mysql.Add(t, mysql.Text("title"))
	t.ContextFilter(mysql.TenantFilter(tenantCol)).ScopeWritesByTenant(tenantCol)
	return t, tenantCol, titleCol
}
...
func TestInsertRefusesATruncatingTenantConversion(t *testing.T) {
	...
	tests := []struct {
		name string
		tbl  *mysql.Table
		vals []mysql.ColumnValue
		ctx  context.Context
	}{
		{
			name: "bound value wider than the ctx tenant",
			tbl:  wide,
			vals: []mysql.ColumnValue{wideTitle.Val("x"), wideTen.Val(big)},
			ctx:  typeCtx(int32(77)),
		},
		{
			name: "ctx tenant wider than the bound value",
			tbl:  narrow,
			vals: []mysql.ColumnValue{narrowTitle.Val("x"), narrowTen.Val(77)},
			ctx:  typeCtx(big),
		},
	}
