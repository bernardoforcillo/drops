...
// WITH moved AS (DELETE FROM scoped RETURNING …) would render with no
// tenant predicate and refuse nothing on a ctx with no tenant.
// Dispatching on a method means the next builder is resolved on the day
// it is written.
//
...
// INSERT row, returning a rebuilt list — or nil when nothing needed
// resolving.
//
