...
// textAxisTable declares the tenant axis as String.
func textAxisTable(name string) (*clickhouse.Table, *clickhouse.Col[string], *clickhouse.Col[string]) {
	t := clickhouse.NewTable(name)
	tenant := clickhouse.Add(t, clickhouse.String("tenantId"))
	title := clickhouse.Add(t, clickhouse.String("title"))
	t.Engine(clickhouse.MergeTree()).OrderBy(tenant).
		ContextFilter(clickhouse.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
...
// losing nothing. The column type is still String — what the guard
// asks about is the Go kind the caller binds, not the SQL type.
func blobAxisTable(name string) (*clickhouse.Table, *clickhouse.Col[[]byte], *clickhouse.Col[string]) {
	t := clickhouse.NewTable(name)
	tenant := clickhouse.Add(t, clickhouse.Custom[[]byte]("tenantId", "String"))
	title := clickhouse.Add(t, clickhouse.String("title"))
	t.Engine(clickhouse.MergeTree()).OrderBy(tenant).
		ContextFilter(clickhouse.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
	return t, tenant, title
...
func TestBytesAndTextAreNotTheSameTenant(t *testing.T) {
	t.Run("bytes bound, text on ctx", func(t *testing.T) {
		...
		_, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val([]byte("acme"))).
			Exec(typeCtx("acme"))
	...
	t.Run("text bound, bytes on ctx", func(t *testing.T) {
		...
		_, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val("acme")).
...
func TestARuneConversionIsRefusedByTheRoundTrip(t *testing.T) {
	...
	_, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val("A")).Exec(typeCtx(65))
	if !errors.Is(err, clickhouse.ErrTenantMismatch) {
		t.Errorf("Exec = %v, want ErrTenantMismatch", err)
	}
	if got := drv.Statements(); len(got) != 0 {
...
func TestANamedStringTypeIsTheSameTenant(t *testing.T) {
	...
	if _, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val("acme")).
		Exec(typeCtx(tenantName("acme"))); err != nil {
		t.Fatalf("Exec: %v", err)
	}
	st := drv.Last()
	checkSQL(t, st.SQL, `INSERT INTO "guard_named_rows" ("tenantId", "title") VALUES (?, ?)`)
	checkArgs(t, st.Args, "acme", "x")
}
