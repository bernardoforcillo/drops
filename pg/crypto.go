...
//
// drops gives you the cipher; you wire it via lifecycle hooks for
// transparent writes or call it directly when you need to control
// the encryption boundary.
//
//	// Construct once at startup with your KMS adapter.
//	cipher := pg.NewEnvelopeCipher(myKMS)
//
//	// Hook into Entity writes — Users.SSN is encrypted before INSERT.
//	UserEntity.Table().OnInsert(pg.EncryptFieldHook[User]("SSN", cipher))
//	UserEntity.Table().OnUpdate(pg.EncryptFieldHook[User]("SSN", cipher))
//
//	// Decrypt after load.
//	u, _ := UserEntity.Get(db, ctx, id)
//	plain, _ := cipher.Decrypt(ctx, []byte(u.SSN))
//
// The cipher is goroutine-safe and allocation-light — the DEK is
// regenerated per call, so reuse the same EnvelopeCipher across the
// process.
...
// without scanning. The format is versioned via the magic byte at
// position 0 so future revisions stay distinguishable.
...
func NewEnvelopeCipher(kms KMS) *EnvelopeCipher {
	if kms == nil {
		panic("drops/pg: NewEnvelopeCipher: kms is nil")
	}
...
// returned bytes are safe to store in a bytea column.
func (c *EnvelopeCipher) Encrypt(ctx context.Context, plaintext []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("drops/pg: EnvelopeCipher.Encrypt: KMS wrap: %w", err)
	}
...
func (c *EnvelopeCipher) Decrypt(ctx context.Context, blob []byte) ([]byte, error) {
	if len(blob) < 1+4 {
		return nil, errors.New("drops/pg: EnvelopeCipher.Decrypt: short header")
	}
	if blob[0] != cipherMagic {
		return nil, fmt.Errorf("drops/pg: EnvelopeCipher.Decrypt: bad magic 0x%x", blob[0])
	}
	wrappedLen := binary.BigEndian.Uint32(blob[1:5])
	if int(wrappedLen)+5 > len(blob) {
		return nil, errors.New("drops/pg: EnvelopeCipher.Decrypt: truncated wrappedDEK")
	}
	wrappedDEK := blob[5 : 5+wrappedLen]
	rest := blob[5+wrappedLen:]
	dek, err := c.kms.Unwrap(ctx, wrappedDEK)
	if err != nil {
		return nil, fmt.Errorf("drops/pg: EnvelopeCipher.Decrypt: KMS unwrap: %w", err)
	}
	block, err := aes.NewCipher(dek)
	if err != nil {
		return nil, err
	...
	if len(rest) < nonceSize+gcm.Overhead() {
		return nil, errors.New("drops/pg: EnvelopeCipher.Decrypt: truncated payload")
	}
...
func NewLocalKMS(key []byte) (*LocalKMS, error) {
	if len(key) != 32 {
		return nil, fmt.Errorf("drops/pg: NewLocalKMS: master key must be 32 bytes, got %d", len(key))
	}
...
func (k *LocalKMS) Unwrap(_ context.Context, wrappedDEK []byte) ([]byte, error) {
	...
	if len(wrappedDEK) < n+k.masterAEAD.Overhead() {
		return nil, errors.New("drops/pg: LocalKMS.Unwrap: truncated wrapped key")
	}
