...
//
//	Invoices := mysql.NewAutoEntity[Invoice]("invoices").
//	    AuthorizeWith(mysql.OwnerGuard{Owner: invoicesTable.Col("createdBy")})
//	ctx = mysql.WithSubject(ctx, currentUserID)
...
// lacking WithSubject.
var ErrSubjectMissing = errors.New("drops/mysql: entity is guarded but ctx has no subject")

// ErrUnauthorized signals a guard explicitly denied the request.
var ErrUnauthorized = errors.New("drops/mysql: unauthorized")
...
func (g OwnerGuard) Predicate(ctx context.Context) (drops.Expression, error) {
	if g.Owner == nil {
		return nil, errors.New("drops/mysql: OwnerGuard.Owner is nil")
	}
...
func (g MembershipGuard) Predicate(ctx context.Context) (drops.Expression, error) {
	if g.Junction == nil || g.JunctionSubject == nil || g.JunctionResource == nil || g.ResourceOwner == nil {
		return nil, errors.New("drops/mysql: MembershipGuard is missing one of Junction / JunctionSubject / JunctionResource / ResourceOwner")
	}
	s, ok := SubjectFrom(ctx)
	if !ok {
		return nil, ErrSubjectMissing
...
func (g anyGuard) Predicate(ctx context.Context) (drops.Expression, error) {
	if len(g.guards) == 0 {
		return nil, errors.New("drops/mysql: AnyOf with no guards")
	}
...
func (g allGuard) Predicate(ctx context.Context) (drops.Expression, error) {
	if len(g.guards) == 0 {
		return nil, errors.New("drops/mysql: AllOf with no guards")
	}
	exprs := make([]drops.Expression, 0, len(g.guards))
	for _, sub := range g.guards {
		e, err := sub.Predicate(ctx)
