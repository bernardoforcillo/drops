...
// valueExpr hands the resolver the expression this binding assigns, so
// a hook that binds a subquery is a statement the walk can reach and
// resolve. Rendering it through WriteSQL instead — which has no ctx —
// is how a hook's SELECT would read every tenant's rows to compute a
// value stored in this one's. This package's ColumnValue requires the
// method; drops/sqlite's does not, so the ported type lacked it.
func (e *exprBinding) valueExpr() drops.Expression { return e.expr }

// rebind restates the assignment against another handle on the same
// column, which this package's ColumnValue requires and drops/sqlite's
// does not. A COPY rather than a mutation: the binding is held by
// pointer, and rebinding in place would move the original assignment's
// column too — the caller is (*UpdateBuilder).Set restating a hook's
// assignment against the statement's own relation, and the hook is
// registered on the table and reused by every write against it.
func (e *exprBinding) rebind(col *Column) ColumnValue {
	cp := *e
	cp.col = col
	return &cp
}
