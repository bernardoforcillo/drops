...
//
// A table may be schema-qualified, and the qualified name is what an
// un-aliased column reference renders, so that is the key: "users"
// declared through NewTable and "billing.users" declared through
// NewSchemaTable are two relations, and a rename keyed on the bare name
// would restate the predicates of one inside a statement over the
// other.
