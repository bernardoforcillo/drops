...
func textAxisTable(name string) (*pg.Table, *pg.Col[string], *pg.Col[string]) {
	t := pg.NewTable(name)
	tenant := pg.Add(t, pg.Text("tenantId").NotNull())
	title := pg.Add(t, pg.Text("title").NotNull())
	t.ContextFilter(pg.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
...
// losing nothing.
func blobAxisTable(name string) (*pg.Table, *pg.Col[[]byte], *pg.Col[string]) {
	t := pg.NewTable(name)
	tenant := pg.Add(t, pg.Bytea("tenantId").NotNull())
	title := pg.Add(t, pg.Text("title").NotNull())
	t.ContextFilter(pg.TenantFilter(tenant)).ScopeWritesByTenant(tenant)
	return t, tenant, title
}
...
func TestARuneConversionIsRefusedByTheRoundTrip(t *testing.T) {
	...
	_, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val("A")).Exec(tenantCtx(65))
	if !errors.Is(err, pg.ErrTenantMismatch) {
		t.Errorf("Exec = %v, want ErrTenantMismatch", err)
	}
	if got := drv.Statements(); len(got) != 0 {
...
func TestANamedStringTypeIsTheSameTenant(t *testing.T) {
	...
	if _, err := db.Insert(tbl).Row(title.Val("x"), tenant.Val("acme")).
		Exec(tenantCtx(tenantName("acme"))); err != nil {
		t.Fatalf("Exec: %v", err)
	}
	st := drv.Last()
	wantSQL := `INSERT INTO "guard_named_rows" ("tenantId", "title") VALUES ($1, $2)`
	if st.SQL != wantSQL {
		t.Errorf("rendered SQL:\n got = %v\nwant = %v", st.SQL, wantSQL)
	}
	if !sameArgs(st.Args, []any{"acme", "x"}) {
		t.Errorf("args:\n got = %#v\nwant = %#v", st.Args, []any{"acme", "x"})
	}
}
