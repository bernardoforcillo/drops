...
// named *SelectBuilder and nothing else would be blind to the other
// three builders — which satisfy drops.Expression exactly as it does,
// and are what a CTE body or a subquery operand is typed as — so
// WITH pruned AS (DELETE FROM scoped …) would render with no tenant
// predicate and refuse nothing on a ctx with no tenant. Dispatching on
// a method means the next builder is resolved on the day it is written.
//
...
// would mean re-ordering placeholders by parsing SQL — which MySQL's
// positional "?" makes worse than PostgreSQL's numbered form rather
// than better, since the order of the arguments is the only thing that
// says which is which. resolveExpr therefore asks for the resolved
// statement itself and lets the surrounding Builder render it.
...
// resolveSets resolves the statements reachable from a SET list or an
// upsert's assignment list, returning a rebuilt list — or nil when
...
// other implementation in it binds a Go value that becomes a parameter.
// The test is an interface rather than the name of one struct so that a
