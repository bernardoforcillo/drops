...
//	    Amount mysql.Money `drop:"amount"`
//	}
//
//	p := Payment{Amount: mysql.MoneyFromString("12.34")}
//	p.Amount = p.Amount.Add(mysql.MoneyFromCents(50))
...
func MoneyFromString(s string) (Money, error) {
	...
	if s == "" {
		return Money{}, errors.New("drops/mysql: MoneyFromString empty")
	}
	neg := false
	if s[0] == '-' {
		neg = true
	...
	if err != nil {
		return Money{}, fmt.Errorf("drops/mysql: MoneyFromString integer: %w", err)
	}
	var frac int64
	if hasFrac && fracPart != "" {
		...
		if err != nil {
			return Money{}, fmt.Errorf("drops/mysql: MoneyFromString fraction: %w", err)
		}
	}
...
func (m *Money) Scan(src any) error {
	...
	case string:
		...
		*m = parsed
	default:
		return fmt.Errorf("drops/mysql: Money.Scan unsupported src %T", src)
	}
	return nil
}
...
func (m Money) assertSameExponent(other Money) {
	if m.Exponent() != other.Exponent() {
		panic(fmt.Sprintf("drops/mysql: Money exponent mismatch (%d vs %d) — normalise first",
