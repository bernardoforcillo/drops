...
//
// The consequence is sharper here than on PostgreSQL: MySQL has no
// transactional DDL, so a migration that drops the column and adds the
// new one cannot be rolled back once the DROP has run. There is nothing
// to undo it with.
//
...
const (
	...
	// RenameColumn renames a column in place — RENAME COLUMN on a
	// server new enough for it, CHANGE COLUMN everywhere else.
...
func (e *RenameAmbiguityError) Error() string {
	...
	b.WriteString("drops/mysql: this schema change could be a rename or a drop-and-add and drops will not guess:\n")
...
// typeFamily reduces a MySQL type name to the family its values live
// in, so varchar(120) and text are the same family and datetime and
// tinyint are not. An ENUM or a SET is its own family, values and all:
// two enums with different members are not the same column.
//
// The input has already been through normaliseType, so it is
// lower-cased and its display widths are gone.
func typeFamily(t string) string {
	t = strings.ToLower(strings.TrimSpace(t))
	for _, mod := range []string{" unsigned", " zerofill", " signed"} {
		t = strings.ReplaceAll(t, mod, "")
	}
	base := t
	...
	case "tinyint", "smallint", "mediumint", "int", "integer", "bigint":
		// tinyint(1) is how MySQL spells a boolean and normaliseType
		// keeps the width for exactly that reason, so it stays out of
		// the integer family: renaming a flag into a counter is not a
...
func validateRenames(prev, cur *Snapshot, renames []Rename) error {
	...
	for _, r := range renames {
		...
		if seenTo[r.Kind.side(r.Table, r.To)] {
			return fmt.Errorf("drops/mysql: two renames both produce %q", r.To)
		}
...
//
// A column is renamed with RENAME COLUMN where the server has it and
// with CHANGE COLUMN where it does not — MySQL learned the first
// spelling in 8.0 and MariaDB in 10.5.2, and a DiffOptions with no
// Server in it means a server of unknown version, which gets the
// spelling both understand. CHANGE COLUMN has to restate the column, so
// it is rendered from the definition the column has *now*: the type
// change, if there is one, belongs to the MODIFY the column diff emits
// afterwards, and doing it here as well would rewrite the table twice.
func renameStatements(prev *Snapshot, renames []Rename, opt DiffOptions) []string {
	var out []string
	for _, r := range renames {
		if r.Kind != RenameTable {
			continue
	...
	renamedTables := applyRenames(prev, tablesOnly(renames))
	for _, r := range renames {
		if r.Kind != RenameColumn {
			continue
		}
		if opt.Server.SupportsRenameColumn() {
			out = append(out, fmt.Sprintf("ALTER TABLE %s RENAME COLUMN %s TO %s;",
				quoteIdent(r.Table), quoteIdent(r.From), quoteIdent(r.To)))
			continue
		}
		def := quoteIdent(r.To)
		if t := renamedTables.Tables[r.Table]; t != nil {
			if c, ok := t.Columns[r.From]; ok {
				under := *c
				under.Name = r.To
				def = columnDefSQL(&under)
			}
		}
		out = append(out, fmt.Sprintf("ALTER TABLE %s CHANGE COLUMN %s %s;",
			quoteIdent(r.Table), quoteIdent(r.From), def))
	}
	return out
}
...
// and every key, index and foreign key that named either following it.
//
...
// produces the rename and the MODIFY and nothing else.
//
// An index keeps the name it was created under, because neither server
// renames one when a column moves, and a CHECK expression keeps its
...
func renameColumnIn(s *Snapshot, table, from, to string) {
	...
	if t.PrimaryKey != nil {
		t.PrimaryKey.Columns = replaceName(t.PrimaryKey.Columns, from, to)
	}
...
func loadRenameLog(fsys fs.FS, dir string) ([]RenameDecision, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("drops/mysql: read rename log: %w", err)
	}
	var l renameLog
	if err := json.Unmarshal(body, &l); err != nil {
		return nil, fmt.Errorf("drops/mysql: parse %s: %w", RenameLogFile, err)
	}
