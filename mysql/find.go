...
func (f *FindBuilder) All(ctx context.Context, dest any) error {
	...
	for _, name := range f.withs {
		if f.table.Relation(name) == nil {
			return fmt.Errorf("drops/mysql: unknown relation %q on table %q", name, f.table.Name())
		}
	}
	...
	if rv.Kind() != reflect.Ptr || rv.Elem().Kind() != reflect.Slice {
		return fmt.Errorf("drops/mysql: Find.All needs a pointer to slice, got %T", dest)
	}
	parentSlice := rv.Elem()
	...
	if structType.Kind() != reflect.Struct {
		return fmt.Errorf("drops/mysql: Find.All needs a slice of struct or *struct, got %s", structType.Kind())
	}
	for _, name := range f.withs {
...
func (f *FindBuilder) One(ctx context.Context, dest any) error {
	...
	if rv.Kind() != reflect.Ptr || rv.Elem().Kind() != reflect.Struct {
		return fmt.Errorf("drops/mysql: Find.One needs a pointer to struct, got %T", dest)
	}
...
func (f *FindBuilder) loadRelation(
	...
	default:
		return fmt.Errorf("drops/mysql: relation %q has unknown kind", rel.Name)
	}
}
...
func (f *FindBuilder) loadSimple(
	...
	if !ok {
		return fmt.Errorf("drops/mysql: relation %q: parent struct missing field for column %q",
			rel.Name, rowKeyCol.Name())
	}
	relField, ok := relationTargetField(parentType, rel.Name)
	if !ok {
		return fmt.Errorf("drops/mysql: relation %q: parent struct has no field tagged dropRel:%q (or matching name)",
			rel.Name, rel.Name)
	}
	...
	if expectsSlice && relFieldType.Kind() != reflect.Slice {
		return fmt.Errorf("drops/mysql: relation %q is HasMany — expected slice field, got %s",
	...
	if childStructType.Kind() != reflect.Struct {
		return fmt.Errorf("drops/mysql: relation %q: target field is neither struct nor slice of struct",
			rel.Name)
	}
	...
	if !ok {
		return fmt.Errorf("drops/mysql: relation %q: target struct %s missing field for column %q",
...
func (f *FindBuilder) loadManyToMany(
	...
	if relFieldType.Kind() != reflect.Slice {
		return fmt.Errorf("drops/mysql: relation %q is ManyToMany — expected slice field, got %s",
			rel.Name, relFieldType.Kind())
	}
	childElemType := relFieldType.Elem()
	childIsPtr := childElemType.Kind() == reflect.Ptr
	...
	if childStructType.Kind() != reflect.Struct {
		return fmt.Errorf("drops/mysql: relation %q: slice element must be struct or *struct",
			rel.Name)
	}
