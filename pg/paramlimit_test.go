...
// The wire carries a Bind message's parameter count in an int16, so
// 65535 is the most a statement can have. It is a limit a bulk write
...
func plEntity(t *testing.T) (*pg.Entity[plRow], *dropstest.Driver, *pg.DB) {
	t.Helper()
	tbl := pg.NewTable("pl_rows")
	pg.Add(tbl, pg.BigSerial("id").PrimaryKey())
	for _, n := range []string{"a", "b", "c", "d", "e", "f", "g"} {
		pg.Add(tbl, pg.Text(n).NotNull())
	}
	drv := dropstest.New()
	return pg.NewEntity[plRow](tbl), drv, pg.New(drv)
}
...
func TestABatchOverTheParameterLimitIsRefusedBeforeItIsSent(t *testing.T) {
	...
	// so 9363 rows is 65541 parameters, six over.
	...
	if !errors.Is(err, pg.ErrTooManyParameters) {
	...
	for _, want := range []string{"65541", "65535", "batches", "CopyFrom"} {
...
func TestAnOversizedInListIsRefusedToo(t *testing.T) {
	...
	var out []struct{}
	err := db.Select(id).From(tbl).Where(pg.In(id, ids)).All(context.Background(), &out)
	if !errors.Is(err, pg.ErrTooManyParameters) {
