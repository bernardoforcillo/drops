...
//	spec := sqlite.NewCursorSpec(
//	    sqlite.OrderKey{Col: Posts.CreatedAt, Desc: true},
//	    sqlite.OrderKey{Col: Posts.ID,        Desc: true}, // tiebreaker
//	)
//
...
//	cur, _ := sqlite.EncodeCursor(spec, last.CreatedAt, last.ID)
//	err = db.Select(/* cols */).From(Posts).
//	    OrderByCursor(spec).
...
func EncodeCursor(spec CursorSpec, values ...any) (Cursor, error) {
	if len(values) != len(spec.Keys) {
		return "", fmt.Errorf("drops/sqlite: EncodeCursor: %d values for %d keys", len(values), len(spec.Keys))
	}
...
func (c Cursor) Decode() ([]any, error) {
	if c == "" {
		return nil, errors.New("drops/sqlite: empty cursor")
	}
	body, err := base64.RawURLEncoding.DecodeString(string(c))
	if err != nil {
		return nil, fmt.Errorf("drops/sqlite: cursor base64: %w", err)
	}
	var w cursorWire
	if err := json.Unmarshal(body, &w); err != nil {
		return nil, fmt.Errorf("drops/sqlite: cursor JSON: %w", err)
	}
...
func decodeCursorValue(cv cursorVal) (any, error) {
	...
	default:
		return nil, fmt.Errorf("drops/sqlite: cursor value type %q unknown", cv.T)
	}
}
...
func (s *SelectBuilder) OrderByCursor(spec CursorSpec) *SelectBuilder {
	for _, k := range spec.Keys {
		s.orderBy = append(s.orderBy, orderKeyExpr(k))
	}
	return s
}
...
func (s *SelectBuilder) AfterCursor(spec CursorSpec, c Cursor) *SelectBuilder {
	...
	if len(values) != len(spec.Keys) {
		s.wheres = append(s.wheres, cursorErrExpr(fmt.Errorf("drops/sqlite: cursor has %d values, spec wants %d", len(values), len(spec.Keys))))
		return s
	}
...
// malformed cursor lands. Saves us from a separate err return on
// every chained builder method.
