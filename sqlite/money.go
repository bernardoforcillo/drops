...
func MoneyFromString(s string) (Money, error) {
	...
	if s == "" {
		return Money{}, errors.New("drops/sqlite: MoneyFromString empty")
	}
	neg := false
	if s[0] == '-' {
		neg = true
	...
	if err != nil {
		return Money{}, fmt.Errorf("drops/sqlite: MoneyFromString integer: %w", err)
	}
	var frac int64
	if hasFrac && fracPart != "" {
		...
		if err != nil {
			return Money{}, fmt.Errorf("drops/sqlite: MoneyFromString fraction: %w", err)
		}
	}
...
func (m *Money) Scan(src any) error {
	...
	case string:
		...
		*m = parsed
	default:
		return fmt.Errorf("drops/sqlite: Money.Scan unsupported src %T", src)
	}
	return nil
}
...
func (m Money) assertSameExponent(other Money) {
	if m.Exponent() != other.Exponent() {
		panic(fmt.Sprintf("drops/sqlite: Money exponent mismatch (%d vs %d) — normalise first",
