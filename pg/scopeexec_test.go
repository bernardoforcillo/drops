...
func executorEntryPoints() []executorCase {
	return []executorCase{
		...
		// authzshadow.go — the guard comparison, which is the one
		// place in the package that deliberately reads AROUND an
		// automatic predicate. It names the guard filter and steps
		// around that one; the tenant axis has to survive the step,
		// in the unguarded read as much as in the guarded one, or
		// "show me the rows my guard hides" would answer with every
		// tenant's.
		{name: "CompareGuard", run: func(f *execFixture, ctx context.Context) error {
			f.ent.AuthorizeWith(pg.OwnerGuard{Owner: f.tbl.Col("id")})
			_, err := pg.CompareGuard(pg.WithSubject(ctx, int64(1)), f.db, f.ent,
				func(execRow) bool { return true }, nil)
			return err
		}},
...
var exemptExecutors = map[string]string{
	...
	"DB.InTxWithTimeout":  "runs the caller's function against a transactional *DB and reuses DB.InTx wholesale, so every statement inside it goes out through one of the executors this table covers. The one it sends of its own is SET LOCAL statement_timeout, before the caller is handed the *DB: it names no relation and selects no row, so there is nothing in it for a tenant predicate to scope. What it establishes is how long the SERVER will spend on each statement in the transaction, which bounds what the statements inside it may cost and is never a substitute for scoping them",
	"DB.InReadTx":         "runs the caller's function against a transactional *DB and reuses DB.InTx wholesale, so every statement inside it goes out through one of the executors this table covers. The one it sends of its own is SET TRANSACTION READ ONLY, before the caller is handed the *DB: it names no relation and selects no row, so there is nothing in it for a tenant predicate to scope. What it establishes is that the server will refuse a write for the rest of the transaction, which narrows what the statements inside it may do and is never a substitute for scoping them",
